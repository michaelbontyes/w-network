class grunt::install{
  ...
  # install npm
  exec { 'npm':
    command => '/usr/bin/wget https://npmjs.org/install.sh | /bin/sh',
  ...
  exec { 'gem install susy': 
   ...
   # install breakpoint
  exec { 'gem install breakpoint': 
    command => '/usr/bin/gem install breakpoint',
    require => Package['ruby1.9.3'],
  }
  # install compass
  exec { 'gem install compass --pre': 
    command => '/usr/bin/gem install compass --pre',
    require => Package['ruby1.9.3'],
  }
